...
def get_locale() -> str:
    if has_request_context():
        raw = request.headers.get('Accept-Language', 'ko')
        return raw if raw in _translations else 'ko'
    return getattr(_thread_local, 'locale', 'ko')


def t(key: str, **kwargs) -> str:
    locale = get_locale()
    messages = _translations.get(locale, _translations.get('ko', {}))
...
def get_language_instruction() -> str:
    locale = get_locale()
    lang_config = _languages.get(locale, _languages.get('ko', {}))
    return lang_config.get('llmInstruction', '한국어로 답변해주세요.')
